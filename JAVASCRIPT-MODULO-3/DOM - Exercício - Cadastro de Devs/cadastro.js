...
botaoCadastro.addEventListener('click', function(evento){
    ...
    const confirmacao = confirm("Você tem certeza que deseja confirmar o envio do formulário? (S/N).")
    if(confirmacao){
        ...
        console.log(containers_tecnologias)

        for (let index = 0; index < containers_tecnologias.length; index++) {
            let nome_tecnologias = document.querySelector(containers_tecnologias[index]).value
            console.log(nome_tecnologias)
    }
